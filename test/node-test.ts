...
describe("resolveNodeImport(root, spec) with top-level node_modules", () => {
  ...
  it("resolves the version of a direct dependency", async () => {
    ...
    assert.deepStrictEqual(await resolveNodeImport(testRoot, "mime"), "/_node/mime@4.0.1/index.js");
  });
  it("allows entry points", async () => {
    assert.deepStrictEqual(await resolveNodeImport(testRoot, "mime/lite"), "/_node/mime@4.0.1/lite.js");
  });
  it("allows non-javascript entry points", async () => {
    assert.deepStrictEqual(await resolveNodeImport(testRoot, "glob/package.json"), "/_node/glob@10.3.10/package.json");
  });
