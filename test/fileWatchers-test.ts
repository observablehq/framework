...
describe("FileWatchers.of(root, path, names, callback)", () => {
  ...
  it("watches multiple files", async () => {
    ...
    try {
      touch(FilePath("test/input/build/files/file-top.csv"));
      touch(FilePath("test/input/build/files/subsection/file-sub.csv"));
  ...
  it("watches a file generated by a data loader", async () => {
    ...
    try {
      touch(FilePath("test/input/build/simple/data.txt.js"));
  ...
  it("ignores changes that don’t affect the modification time", async () => {
    const then = new Date();
    touch(FilePath("test/input/build/files/file-top.csv"), then);
    const [watcher, watches] = await useWatcher(root, "files.md", ["file-top.csv"]);
    try {
      touch(FilePath("test/input/build/files/file-top.csv"), then);
  ...
  it("ignores changes to empty files", async () => {
    ...
    try {
      touch(FilePath("test/input/empty.js"), then);
      assert.deepStrictEqual(await watches(10), []);
    } finally {
      watcher.close();
    }
  ...
  it("handles a file being renamed", async () => {
    writeFileSync(FilePath("test/input/build/files/temp.csv"), "hello", "utf-8");
    try {
      ...
      try {
        ...
        renameSync(FilePath("test/input/build/files/temp.csv"), FilePath("test/input/build/files/temp2.csv"));
        writeFileSync(FilePath("test/input/build/files/temp.csv"), "hello 2", "utf-8");
        assert.deepStrictEqual(await watches(), ["temp.csv"]);

        // Then test that writing to the original location watches the new file.
        writeFileSync(FilePath("test/input/build/files/temp.csv"), "hello 3", "utf-8");
        assert.deepStrictEqual(await watches(), ["temp.csv"]);
      } finally {
        watcher.close();
      }
    } finally {
      cleanupSync(FilePath("test/input/build/files/temp.csv"));
      cleanupSync(FilePath("test/input/build/files/temp2.csv"));
    }
  });
  it("handles a file being renamed and removed", async () => {
    ...
    try {
      ...
      try {
        ...
        unlinkSync(FilePath("test/input/build/files/temp.csv"));
        await pause();
...
function touch(path: FilePath, date = new Date()): void {
  utimesSync(path, date, date);
}

function cleanupSync(path: FilePath): void {
  try {
    unlinkSync(path);
  } catch {
    // ignore
