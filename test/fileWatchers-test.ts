...
import {renameSync, unlinkSync, utimesSync, writeFileSync} from "../src/normalizedFs.js";
...
describe("FileWatchers.of(root, path, names, callback)", () => {
  ...
  it("watches a file within a static archive", async () => {
    ...
    try {
      touch(`test/input/build/archives.${platform}/static.zip`);
  ...
  it("watches a file within an archive created by a data loader", async function () {
    ...
    try {
      touch(`test/input/build/archives.${platform}/dynamic.zip.sh`);
...
async function useWatcher(
  ...
  const root = rootStr;
  const path = pathStr;
  const names = namesStr.map((name) => name);
