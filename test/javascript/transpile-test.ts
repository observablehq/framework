...
async function testFile(target: string, path: string, sourcePath = path): Promise<string> {
  const input = `import {FileAttachment} from "npm:@observablehq/stdlib";\nFileAttachment(${JSON.stringify(target)})`;
  const output = await transpileModule(input, "docs", path, sourcePath);
  return output.split("\n").pop()!;
}

describe("transpileModule(input, root, path, sourcePath)", () => {
  it("rewrites relative files with import.meta.resolve", async () => {
    assert.strictEqual(await testFile("./test.txt", "_import/test.js", "test.js"), 'FileAttachment("../test.txt", import.meta.url)'); // prettier-ignore
    assert.strictEqual(await testFile("./sub/test.txt", "_import/test.js", "test.js"), 'FileAttachment("../sub/test.txt", import.meta.url)'); // prettier-ignore
    assert.strictEqual(await testFile("./test.txt", "_import/sub/test.js", "sub/test.js"), 'FileAttachment("../../sub/test.txt", import.meta.url)'); // prettier-ignore
    assert.strictEqual(await testFile("../test.txt", "_import/sub/test.js", "sub/test.js"), 'FileAttachment("../../test.txt", import.meta.url)'); // prettier-ignore
  });
...
describe("transpileModule(input, root, path)", () => {
  ...
  it("ignores FileAttachment if masked by a reference", async () => {
    ...
    const output = (await transpileModule(input, "docs", "test.js")).split("\n").pop()!;
  ...
  it("throws a syntax error with non-literal calls", async () => {
    const input = "import {FileAttachment} from \"npm:@observablehq/stdlib\";\nFileAttachment(`./${'test'}.txt`)";
    await assert.rejects(() => transpileModule(input, "docs", "test.js"), /FileAttachment requires a single literal string/); // prettier-ignore
  });
  it("throws a syntax error with URL fetches", async () => {
    const input = 'import {FileAttachment} from "npm:@observablehq/stdlib";\nFileAttachment("https://example.com")';
    await assert.rejects(() => transpileModule(input, "docs", "test.js"), /non-local file path/); // prettier-ignore
  });
  it("ignores non-local path fetches", async () => {
    ...
    await assert.rejects(() => transpileModule(input1, "docs", "test.js"), /non-local file path/); // prettier-ignore
    await assert.rejects(() => transpileModule(input2, "docs", "test.js"), /non-local file path/); // prettier-ignore
    await assert.rejects(() => transpileModule(input3, "docs", "sub/test.js"), /non-local file path/); // prettier-ignore
    await assert.rejects(() => transpileModule(input4, "docs", "sub/test.js"), /non-local file path/); // prettier-ignore
  });
});
