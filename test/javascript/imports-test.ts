...
describe("parseLocalImports(root, paths)", () => {
  it("finds all local imports in one file", () => {
    assert.deepStrictEqual(
      parseLocalImports(FilePath("test/input/build/imports"), [UrlPath("foo/foo.js")]).imports.sort(order),
      [
  ...
  it("finds all local imports in multiple files", () => {
    assert.deepStrictEqual(
      parseLocalImports(FilePath("test/input/imports"), [
        UrlPath("transitive-static-import.js"),
        UrlPath("dynamic-import.js")
      ]).imports.sort(order),
      [
  ...
  it("ignores missing files", () => {
    assert.deepStrictEqual(
      parseLocalImports(FilePath("test/input/imports"), [
        UrlPath("static-import.js"),
        UrlPath("does-not-exist.js")
      ]).imports.sort(order),
      [
        {name: "bar.js", type: "local"},
  ...
  it("find all local fetches in one file", () => {
    assert.deepStrictEqual(
      parseLocalImports(FilePath("test/input/build/fetches"), [UrlPath("foo/foo.js")]).features.sort(order),
      [
  ...
  it("find all local fetches via transitive import", () => {
    assert.deepStrictEqual(
      parseLocalImports(FilePath("test/input/build/fetches"), [UrlPath("top.js")]).features.sort(order),
      [
        {name: "foo/foo-data.csv", type: "FileAttachment"},
        {name: "foo/foo-data.json", type: "FileAttachment"},
...
async function testFile(target: string, path: string): Promise<string> {
  ...
  const output = await rewriteModule(input, FilePath(path), async (path, specifier) => UrlPath(specifier));
  return output.split("\n").pop()!;
}
...
describe("rewriteModule(input, path, resolver)", () => {
  ...
  it("ignores FileAttachment if masked by a reference", async () => {
    ...
    const output = (await rewriteModule(input, FilePath("test.js"), async (path, specifier) => UrlPath(specifier)))
      .split("\n")
      .pop()!;
  ...
  it("throws a syntax error with non-literal calls", async () => {
    const input = "import {FileAttachment} from \"npm:@observablehq/stdlib\";\nFileAttachment(`./${'test'}.txt`)";
    await assert.rejects(() => rewriteModule(input, FilePath("test.js"), async (path, specifier) => UrlPath(specifier)), /FileAttachment requires a single literal string/); // prettier-ignore
  });
  it("throws a syntax error with URL fetches", async () => {
    const input = 'import {FileAttachment} from "npm:@observablehq/stdlib";\nFileAttachment("https://example.com")';
    await assert.rejects(() => rewriteModule(input, FilePath("test.js"), async (path, specifier) => UrlPath(specifier)), /non-local file path/); // prettier-ignore
  });
  it("ignores non-local path fetches", async () => {
    ...
    await assert.rejects(() => rewriteModule(input1, FilePath("test.js"), async (path, specifier) => UrlPath(specifier)), /non-local file path/); // prettier-ignore
    await assert.rejects(() => rewriteModule(input2, FilePath("test.js"), async (path, specifier) => UrlPath(specifier)), /non-local file path/); // prettier-ignore
    await assert.rejects(() => rewriteModule(input3, FilePath("sub/test.js"), async (path, specifier) => UrlPath(specifier)), /non-local file path/); // prettier-ignore
    await assert.rejects(() => rewriteModule(input4, FilePath("sub/test.js"), async (path, specifier) => UrlPath(specifier)), /non-local file path/); // prettier-ignore
  });
});
