...
describe("isLocalImport", () => {
  it("identifies a local import", async () => {
    ...
    assert.equal(resolvePath(root, sourcePath, UrlPath(importValue)), FilePath("docs/helpers.js"));
  ...
  it("relative paths are correctly handled", async () => {
    ...
    const importValue = "./helpers.js";
    assert.equal(resolvePath(root, sourcePath, UrlPath(importValue)), FilePath("docs/subDocs/helpers.js"));
    assert(isLocalImport(importValue, sourcePath));
  });
  ...
  it("identifies a local import from a nested sourcePath", async () => {
    ...
    assert.equal(resolvePath(root, sourcePath, UrlPath(importValue)), FilePath("docs/random.js"));
    assert(isLocalImport(importValue, sourcePath));
  });
  it("cannot go to an ancestor directory beyond the root", async () => {
    ...
    assert.equal(resolvePath(root, sourcePath, UrlPath(importValue2)), FilePath("../random.js"));
    assert.equal(isLocalImport(importValue2, sourcePath), false);
    const importValue3 = "/../random.js";
    assert.equal(resolvePath(root, sourcePath, UrlPath(importValue3)), FilePath("random.js"));
