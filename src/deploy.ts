...
class DeployBuildEffects implements BuildEffects {
  ...
  async copyFile(sourcePath: string, outputPath: string) {
    ...
    try {
      ...
      // 413 is "Payload Too Large", however sometimes Cloudflare returns a custom Cloudflare error, 520. Handle both.
      if (isHttpError(error) && (error.statusCode === 413 || error.statusCode === 520)) {
