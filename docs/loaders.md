...
If you use `.py`, `.R`, `.rs`, `.go`, or `.php`, the corresponding interpreter (`python3`, `Rscript`, `rust-script`, `go run`, or `php`, respectively) must be installed and available on your `$PATH`. Any additional modules, packages, libraries, _etc._, must also be installed before you can use them.

Whereas `.js`, `.ts`, `.py`, `.R`, `.rs`, `.go`, `.sh` and `.php` data loaders are run via interpreters, `.exe` data loaders are run directly and must have the executable bit set. This is typically done via [`chmod`](https://en.wikipedia.org/wiki/Chmod). For example:
