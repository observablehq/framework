...
Downloads from npm are cached in `.observablehq/cache/_npm` within your [source root](./config#root) (`pages` by default). An imported module is downloaded from jsDelivr only if it is not already in the cache. You can clear the cache and restart the server to re-fetch the latest versions of libraries from npm.
...
Imports from `node_modules` are cached in `.observablehq/cache/_node` within your [source root](./config#root) (`pages` by default). You shouldn’t need to clear this cache as it is automatically managed, but feel free to clear it you like.
